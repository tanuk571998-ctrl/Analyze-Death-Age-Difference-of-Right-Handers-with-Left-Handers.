...
Original file is located at
    https://colab.research.google.com/drive/1SDWqKOBqK7Nyyy-c-XY07MChOoCQN-Yb
...
fig, ax = plt.subplots()
ax.plot(lefthanded_data['Birth_year'], lefthanded_data['Mean_lh'], label='Mean Left-handedness', marker='.') # plot 'Mean_lh' vs. 'Birth_year'
ax.set_xlabel('Birth Year') # set the x label for the plot
ax.set_ylabel('Left-handedness rate (%)') # set the y label for the plot
ax.legend()
plt.show()
...
# Convert 'Age' column to numeric, coercing errors to NaN, then drop NaNs
death_distribution_data['Age'] = pd.to_numeric(death_distribution_data['Age'], errors='coerce')
death_distribution_data = death_distribution_data.dropna(subset=['Age', 'Both Sexes'])

# plot number of people who died as a function of age
fig, ax = plt.subplots()
ax.plot(death_distribution_data['Age'], death_distribution_data['Both Sexes'], data = death_distribution_data, marker='o') # plot 'Both Sexes' vs. 'Age'
ax.set_xlabel('Age')
...
def P_A_given_lh(ages_of_death, death_distribution_data, study_year = 1990):
    ...
    # Create a mapping from age to P(A) for all available ages in death_distribution_data
    age_to_P_A = pd.Series(death_distribution_data['Both Sexes'].values / total_deaths, index=death_distribution_data['Age'])
    # Select P(A) values corresponding to the `ages_of_death` input, ensuring alignment
    P_A = age_to_P_A.reindex(ages_of_death, fill_value=0).values # Convert to numpy array
...
# Ensure lefthanded_data has 'Birth_year' and 'Mean_lh' columns
# This part is added to ensure data integrity, in case previous cells were not run
if 'Birth_year' not in lefthanded_data.columns:
    lefthanded_data['Birth_year'] = 1986 - lefthanded_data['Age']
if 'Mean_lh' not in lefthanded_data.columns:
    lefthanded_data['Mean_lh'] = (lefthanded_data['Male'] + lefthanded_data['Female'])/2
...
print("Average age of death for left-handed individuals: " + str(round(average_lh_age, 1)) + " years.")
print("Average age of death for right-handed individuals: " + str(round(average_rh_age, 1)) + " years.")
